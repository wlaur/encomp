fn main() {
    ...
    // it does NOT invalidate the (expensive) polars dependency build.
    // pyo3 `extension-module` leaves the Python symbols undefined (resolved at
    // runtime from the host process). macOS rejects undefined symbols at link
    // time unless we allow dynamic lookup. (libCoolProp is dlopen'd at runtime,
    // so no link/rpath to it is needed.) Linux allows undefined symbols in .so by
    // default; Windows pyo3 links python -- so this flag is macOS-only.
