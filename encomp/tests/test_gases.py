...
def test_gas_conversion():
    ...
    assert isinstance(
        mass_from_actual_volume(V, (P, T)),
        Q[Mass]
    )

    assert isinstance(
        mass_to_actual_volume(m, (P, T)),
        Q[Volume]
    )

    assert isinstance(
        actual_volume_to_normal_volume(V, (P, T)),
        Q[Volume]
    )

    assert isinstance(
        normal_volume_to_actual_volume(V, (P, T)),
        Q[Volume]
    )
